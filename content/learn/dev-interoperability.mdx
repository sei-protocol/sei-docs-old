...
Precompiled EVM contracts and Pointer Contracts enable existing CosmWasm
contracts and Cosmos-SDK modules to be accessible from the EVM. New smart
contracts on Sei should be deployed on the EVM directly.
...
ERC721, etc.) and for already-deployed CosmWasm contracts (CW20, CW721, CW1155).
Note that Prop 115 disables new CosmWasm deployments, so pointer creation on the
CW side is now mainly a tool for surfacing legacy assets on the EVM.
