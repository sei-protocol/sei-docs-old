...
| 3. Signed Message | Medium | Sign a predefined message to prove ownership |
| 4. Public Key | Low | Provide a compressed public key for association |
| 5. Gasless Signed Message | Low | Sign a message without requiring gas (if funded) |

</div>

<Info>Using any of these methods will ensure the **public key** is known to the chain, enabling automatic association between the EVM-compatible and Bech32 addresses.</Info>
...
<Info> **Security Risk**: **Low** – Requires signing a message but does not expose the private key. No gas is consumed if the account already has funds.</Info>
...
<Info>**Note**: The account must have at least 1 wei to perform this operation.</Info>
