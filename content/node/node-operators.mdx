...
For the *full* unmodified default `app.toml`, `config.toml`, and `client.toml`
shipped by the latest tagged `seid` release, see the auto-generated
[Default Config Reference](/node/default-configs). The snippets below are
opinionated tuning recommendations layered on top of those defaults.
